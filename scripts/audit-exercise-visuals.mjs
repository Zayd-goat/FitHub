...
function evaluateTypeScript(file, dependencies = {}) {
  ...
  const exportedNames = [...source.matchAll(/\bexport\s+(?:const|function|class)\s+([A-Za-z_$][\w$]*)/g)].map((match) => match[1]);
  const withoutImports = source.replace(/^import\s+(?:type\s+)?[\s\S]*?from\s+['"][^'"]+['"];?\s*$/gm, '');
  const withoutExports = withoutImports.replace(/\bexport\s+(?=(?:const|function|class|type|interface)\b)/g, '');
  const code = stripTypeScriptTypes(withoutExports, { mode: 'transform', sourceMap: false });
  ...
  return Function('require', '__filename', '__dirname', `${code}\nreturn { ${exportedNames.join(', ')} };`)(
    localRequire,
    file,
    path.dirname(file),
  );
...
const report = {
  ...
  release_ready: exercises.length === 229 && assetRows.length === 472 && male.size === 236 && female.size === 236 && runtimeIssues.length === 0 && parityIssues.length === 0 && unreferencedFiles.length === 0 && duplicateGroups.length === 0 && pendingDedicated.length === 0,
...
if (exercises.length !== 229) throw new Error(`Expected 229 unique exercises, found ${exercises.length}`);
