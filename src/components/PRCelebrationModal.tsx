  ...
  return <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
    <View style={s.overlay}>
      ...
      <View style={s.card}>
        ...
        {events.flatMap(e=>e.new_clubs??[]).map(club=><View key={club.club_key} style={s.clubUnlock}><Text style={s.clubTitle}>★ {club.previous_threshold_kg?'CLUB LEVEL UP':'NEW CLUB UNLOCKED'}</Text><Text style={s.clubText}>You are now part of the {formatWeight(club.threshold_kg,weightUnit,0)} {club.exercise_name} Club.</Text>{club.previous_threshold_kg?<Text style={s.clubMeta}>Your {formatWeight(club.previous_threshold_kg,weightUnit,0)} club membership has moved up.</Text>:null}<Text style={s.clubMeta}>{Number(club.active_member_count??1).toLocaleString()} active member{Number(club.active_member_count??1)===1?'':'s'} currently in this club</Text></View>)}
