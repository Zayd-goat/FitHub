...
type Page = 'main' | 'progress' | 'history' | 'daily' | 'journey' | 'clubs' | 'supplements' | 'split' | 'customize' | 'sharedGym' | 'notifications';
  ...
  return <View style={styles.safe}>
    ...
    <View style={styles.content}>
      {page === 'progress' ? <ProgressScreen profile={profile} focus={progressFocus} onBack={() => setPage('main')} /> :
       ...
       /> : <>
        {tab === 'home' && <DashboardTab profile={profile} unreadNotifications={unreadNotifications} onOpenNotifications={()=>setPage('notifications')} onStartWorkout={() => chooseTab('workout')} onViewProgress={openProgress} onViewWorkouts={openHistory} onOpenJourney={openJourney} onOpenSupplements={()=>setPage('supplements')} onOpenFood={()=>chooseTab('food')} onOpenFriends={()=>chooseTab('friends')} onOpenSettings={()=>setPage('customize')} onOpenSplit={()=>setPage('split')} onOpenChallenges={()=>chooseTab('friends')} />}
