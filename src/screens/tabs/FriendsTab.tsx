...
type FriendsView = 'feed' | 'following' | 'invites';
...
const FriendsTab = forwardRef<FriendsTabHandle, { profile: Profile }>(function FriendsTab({ profile }, ref) {
  ...
  const load = async () => {
    const [req, fr, gi, prefs] = await Promise.all([
  ...
  const sendGymInvite = async () => {
    ...
    try {
      ...
      if (error) throw error;
      // Flush this invite's push outbox immediately. The server validates that
      // the signed-in caller created the invite; cron retries if delivery fails.
      await supabase.functions.invoke('friend-notifications', { body: { invite_id: data.id } }).catch(() => null);
      setInviteNote('');
      Alert.alert('Gym invite sent', `${friend?.username ?? 'Your friend'} has been notified and can accept or decline it in FitHub.`);
      load();
    } catch (error: any) {
