...
export default function DashboardTab({ profile, onStartWorkout, onViewProgress, onViewWorkouts, onViewDailyActivity, onOpenJourney, onOpenSupplements, onOpenFood, onOpenFriends }: Props) {
  ...
  return <ScrollView contentContainerStyle={styles.wrap}>
    ...
    <View style={styles.weekMetrics}>
      <Stat label="TODAY'S VOLUME" value={todayVolume ? `${kgToDisplay(todayVolume, weightUnit).toLocaleString()} ${weightUnit}` : '—'} sub="completed sets" accent={colors.primary} icon="▥" onPress={()=>onViewDailyActivity('volume')}/>
      <Stat label="WEEKLY PRS" value={String(weekPrs)} sub={`${monthPrs} this month`} accent="#EF3B45" icon="↗" onPress={()=>onViewProgress('prs')}/>
      <Stat label={locked?'ACTIVE DAYS':'EST. ENERGY'} value={locked?String(weekWorkouts):(todayEnergy?String(todayEnergy):'—')} sub={locked?'this week':todayEnergy?'today':'no session yet'} accent={colors.blue} icon={locked?'●':'⚡'} onPress={()=>onViewDailyActivity(locked?'volume':'energy')}/>
    </View>
...
const createStyles = (colors: any) => StyleSheet.create({
  ...
  weekMetrics:{flexDirection:'row',gap:7,marginTop:9,marginBottom:10},
