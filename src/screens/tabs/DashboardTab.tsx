      ...
      <RefreshableScrollView onRefresh={load} contentContainerStyle={styles.wrap} showsVerticalScrollIndicator={false}>
        ...
        <Pressable onPress={() => onOpenJourney('week')} style={({ pressed }) => pressed && styles.pressed}>
          <View style={styles.weekCard}>
            ...
            <View style={styles.weekProgressRow}>
              ...
              <View style={styles.progressCopy}><Text style={styles.progressLabel}>WORKOUTS{`\n`}COMPLETED</Text><Text style={styles.progressValue}>{weekWorkouts}<Text style={styles.progressGoal}>/{weeklyWorkoutGoal}</Text></Text><Text style={styles.weekSummaryLink}>View Full Journey ›</Text></View>
              <ProgressRing value={weekMinutes} goal={300} icon={<Text style={styles.bolt}>ϟ</Text>} />
        ...
        <Pressable onPress={onOpenFriends} style={({ pressed }) => [styles.feedCard, pressed && styles.pressed]}>
          <View style={styles.feedColumn}>
            {(homeActivities.length ? homeActivities.slice(0, 3) : homeFriends.slice(0, 3)).map((item: any, index: number) => <ActivityRow key={item.id ?? item.user_id ?? index} item={item} compact />)}
          ...
          <View style={styles.feedColumn}>
            {(homeActivities[3] || homeActivities[0] || homeFriends[0]) ? <ActivityRow item={homeActivities[3] || homeActivities[0] || homeFriends[0]} /> : <Text style={styles.feedEmpty}>Friend activity will appear here.</Text>}
            <View style={styles.openFeedRow}><Text style={styles.openFeedIcon}>▧</Text><Text style={styles.feedLink}>Open Friend Feed ›</Text></View>
          </View>
        </Pressable>
...
function ActivityRow({ item, compact = false }: { item: any; compact?: boolean }) {
  const { colors, isDark } = useTheme();
  const styles = createStyles(colors, isDark);
  const username = String(item.username ?? 'Friend');
  const description = item.summary ? `${username} finished ${sessionTitle(item.summary)}` : `${username} is ready to train`;
  return <View style={[styles.activityRow, compact && styles.activityRowCompact]}><View style={styles.activityAvatar}>{item.avatar_url ? <Image source={{ uri: item.avatar_url }} style={styles.friendAvatar}/> : <Text style={styles.friendInitial}>{username[0]?.toUpperCase()}</Text>}<View style={[styles.activityDot, { backgroundColor: item.summary ? colors.green : colors.gold }]}/></View><View style={{ flex: 1 }}><Text style={styles.activityName}>{username}</Text><Text style={styles.activityText} numberOfLines={compact ? 1 : 2}>{description}</Text></View></View>;
}
...
const createStyles = (colors: any, isDark: boolean) => StyleSheet.create({
  ...
  feedCard: { minHeight: 152, borderRadius: 18, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.panel, flexDirection: 'row', alignItems: 'stretch', padding: 14, marginBottom: 9, shadowColor: colors.shadow, shadowOpacity: isDark ? .22 : .11, shadowRadius: 9, elevation: 3 }, feedColumn: { flex: 1, justifyContent: 'space-between', gap: 7 }, feedDivider: { width: 1, backgroundColor: colors.border, marginHorizontal: 11 }, activityRow: { flexDirection: 'row', alignItems: 'center', gap: 8, minHeight: 54 }, activityRowCompact: { minHeight: 34 }, activityAvatar: { width: 38, height: 38, borderRadius: 19, backgroundColor: colors.panel2, alignItems: 'center', justifyContent: 'center', position: 'relative' }, friendAvatar: { width: '100%', height: '100%', borderRadius: 999 }, friendInitial: { color: colors.text, fontWeight: '900' }, activityDot: { position: 'absolute', right: -1, bottom: 0, width: 10, height: 10, borderRadius: 5, borderWidth: 2, borderColor: colors.panel }, activityName: { color: colors.text, fontSize: 11, fontWeight: '900' }, activityText: { color: colors.muted, fontSize: 9, lineHeight: 12, marginTop: 1 }, feedEmpty: { color: colors.muted, fontSize: 10, lineHeight: 14 }, openFeedRow: { borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 10, flexDirection: 'row', alignItems: 'center', gap: 7 }, openFeedIcon: { color: colors.text, fontSize: 18 }, feedLink: { color: colors.text, fontSize: 10, fontWeight: '900' },
