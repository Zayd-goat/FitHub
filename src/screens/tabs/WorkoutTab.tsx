  ...
  return (
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.wrap} keyboardShouldPersistTaps="handled">
        ...
        ].map(muscle=>{const count=exerciseLibrary.filter(ex=>muscle.label==='Arms'?['Biceps','Triceps','Forearms'].includes(ex.targetArea):ex.targetArea===muscle.label).length;return <Pressable key={muscle.label} onPress={()=>setMuscleFilter(muscle.label)} style={({pressed})=>[styles.muscleGridCard,pressed&&{opacity:.72}]}><View style={styles.muscleImageStage}><Image source={muscle.image} style={muscle.label==='Cardio'?styles.muscleGridCardio:styles.muscleGridImage}/></View><View style={styles.muscleCardFooter}><View><Text style={styles.muscleGridLabel}>{muscle.label}</Text><Text style={styles.muscleGridCount}>{count} exercises</Text></View><Text style={styles.muscleGridArrow}>›</Text></View></Pressable>})}</View> : <>
      ...
      <View style={styles.previewBackdrop}>
        <View style={styles.previewSheet}>
          <View style={styles.previewHeader}>
            <View><Text style={styles.previewEyebrow}>WORKOUT PREVIEW</Text><Text style={styles.previewTitle}>{templateName || 'Your workout'}</Text><Text style={styles.previewMeta}>{builder.length} exercise{builder.length===1?'':'s'} • review before starting</Text></View>
            <Pressable onPress={()=>setShowWorkoutPreview(false)}><Text style={styles.previewClose}>×</Text></Pressable>
          </View>
          <ScrollView style={{maxHeight:430}} contentContainerStyle={{gap:9,paddingVertical:8}}>
            {builder.map((item,index)=>{const img=imageForExercise(item.exercise,profile.gender);return <View key={item.id} style={styles.previewRow}>{img?<Image source={img} style={styles.previewThumb}/>:null}<View style={{flex:1}}><Text style={styles.previewIndex}>EXERCISE {index+1}</Text><Text style={styles.previewName}>{item.exercise.name}</Text><Text style={styles.previewExerciseMeta}>{item.exercise.equipment} • {item.exercise.metric_type==='strength'?`${item.strengthSets.length} sets`:item.exercise.metric_type==='distance'?'distance / duration':'duration'}</Text></View></View>})}
          </ScrollView>
...
const createStyles = (colors: any) => StyleSheet.create({
  ...
  muscleGridCard: { width: '48.5%', minHeight: 218, borderRadius: 18, overflow: 'hidden', backgroundColor: colors.panel, borderWidth: 1, borderColor: colors.border, shadowColor: colors.shadow, shadowOpacity:.12, shadowRadius:10, shadowOffset:{width:0,height:4}, elevation:2 },
  muscleImageStage:{height:164,alignItems:'center',justifyContent:'center',backgroundColor:colors.panel2,padding:5},
  muscleGridImage: { width: '100%', height: '100%', resizeMode: 'contain' },
  muscleGridCardio: { width: '100%', height: '100%', resizeMode: 'contain' },
  muscleShade: { display:'none' },
  muscleCardFooter:{minHeight:54,flexDirection:'row',alignItems:'center',justifyContent:'space-between',paddingHorizontal:13,paddingVertical:8,backgroundColor:colors.panel},
  muscleGridLabel: { color: colors.text, fontSize: 17, fontWeight: '900' },
  muscleGridCount:{color:colors.muted,fontSize:10,marginTop:2},
  muscleGridArrow:{color:colors.text,fontSize:29,fontWeight:'300'},
  ...
  previewButton:{borderWidth:1.5,borderColor:colors.primary,borderRadius:9,minHeight:42,alignItems:'center',justifyContent:'center',backgroundColor:colors.primarySoft,marginBottom:8},
  ...
  exerciseList: { gap: 10 },
  exerciseRow: { minHeight: 112, flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: colors.panel, borderWidth: 1, borderColor: colors.border, borderRadius: 18, padding: 10, overflow: 'hidden', shadowColor:colors.shadow,shadowOpacity:.08,shadowRadius:7,shadowOffset:{width:0,height:3},elevation:1 },
  thumb: { width: 126, height: 91, resizeMode: 'contain', borderRadius: 13, backgroundColor: colors.panel2 },
  cardioThumb: { width: 126, height: 91, resizeMode: 'contain', borderRadius: 13, backgroundColor: colors.panel2 },
  ...
  target: { color: '#EF3B45', fontWeight: '900', fontSize: 11 },
  exName: { color: colors.text, fontWeight: '900', fontSize: 15, marginTop: 2 },
  exMeta: { color: colors.muted, fontSize: 10, marginTop: 4, lineHeight:14 },
  plus: { width: 42, height: 42, borderRadius: 21, borderWidth: 1.8, borderColor: colors.primary, alignItems: 'center', justifyContent: 'center' },
  plusSelected: { backgroundColor: colors.primary, borderColor: colors.primary },
  plusText: { fontWeight: '900', fontSize: 20, lineHeight: 22 },
  previewBackdrop:{flex:1,backgroundColor:'rgba(0,0,0,.55)',justifyContent:'flex-end'},
  previewSheet:{backgroundColor:colors.bg,borderTopLeftRadius:26,borderTopRightRadius:26,padding:18,paddingBottom:28,borderWidth:1,borderColor:colors.border},
  previewHeader:{flexDirection:'row',justifyContent:'space-between',alignItems:'flex-start',gap:12},
  previewEyebrow:{color:colors.primary,fontSize:10,fontWeight:'900'},previewTitle:{color:colors.text,fontSize:24,fontWeight:'900',marginTop:3},previewMeta:{color:colors.muted,fontSize:11,marginTop:3},previewClose:{color:colors.text,fontSize:32,lineHeight:34},
  previewRow:{minHeight:92,borderRadius:16,borderWidth:1,borderColor:colors.border,backgroundColor:colors.panel,flexDirection:'row',alignItems:'center',gap:12,padding:9},previewThumb:{width:96,height:74,resizeMode:'contain',borderRadius:12,backgroundColor:colors.panel2},previewIndex:{color:'#EF3B45',fontSize:9,fontWeight:'900'},previewName:{color:colors.text,fontSize:15,fontWeight:'900',marginTop:2},previewExerciseMeta:{color:colors.muted,fontSize:10,marginTop:3},previewActions:{gap:8,marginTop:12},
