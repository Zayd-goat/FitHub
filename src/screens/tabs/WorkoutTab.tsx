...
const WorkoutTab = forwardRef<WorkoutTabHandle, {
  ...
  useImperativeHandle(ref,()=>({goBack:()=>{
    if(showReorder){setShowReorder(false);return true;}
    ...
    return false;
  }}),[showReorder,showExerciseGuide,showExercisePicker,showSaveForm,showSavedWorkouts,screen,muscleFilter]);
  ...
  if (screen === 'active') {
    ...
    const guide = current ? movementGuide(current.exercise) : null;
    return (
      <>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={pullRefresh} colors={[colors.primary]} tintColor={colors.primary} progressBackgroundColor={colors.panel}/>} contentContainerStyle={styles.activeWrap} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.orderHintRow}><Text style={styles.orderHint}>Hold an exercise to change its position</Text><Pressable onPress={()=>setShowReorder(true)} style={styles.orderButton}><Text style={styles.orderButtonText}>REORDER</Text></Pressable></View>
            ...
            {builder.map((item, index) => {
              ...
              return (
                <Pressable
                  ...
                  onLongPress={() => setShowReorder(true)}
                  delayLongPress={350}
                  style={[
        ...
        <Modal visible={showReorder} animationType="slide" transparent onRequestClose={() => setShowReorder(false)}>
          <View style={styles.modalShade}>
            <View style={styles.reorderSheet}>
              <View style={styles.pickerHeader}>
                <View style={{flex:1}}><Text style={styles.pickerTitle}>Exercise order</Text><Text style={styles.pickerSubtitle}>{activeSharedSessionId&&!activeSharedLeader?'Only the session leader can reorder this shared workout.':'Hold the grip and drag a row. You can also move it next or to the end.'}</Text></View>
                <Pressable onPress={()=>setShowReorder(false)}><Text style={styles.pickerClose}>×</Text></Pressable>
              </View>
              <ScrollView contentContainerStyle={styles.reorderList} scrollEnabled>
                {builder.map((item,index)=><DraggableOrderRow key={item.id} item={item} index={index} count={builder.length} isCurrent={index===activeExerciseIndex} disabled={Boolean(activeSharedSessionId&&!activeSharedLeader)} onMove={moveExercise} onMoveNext={()=>moveExercise(index,Math.min(builder.length-1,index<activeExerciseIndex?activeExerciseIndex:activeExerciseIndex+1))} onMoveEnd={()=>moveExercise(index,builder.length-1)}/>) }
              </ScrollView>
              <Button title="DONE" onPress={()=>setShowReorder(false)}/>
            </View>
          </View>
        </Modal>
            ...
            <View style={styles.guideSheet}>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                {current ? <>
                  ...
                  {guide ? <><GuideSteps title="SETUP" items={guide.setup}/><GuideSteps title="PERFORM THE MOVEMENT" items={guide.perform}/><GuideSteps title="BREATHING & CONTROL" items={[guide.breathing]}/><GuideSteps title="COMMON MISTAKES" items={guide.mistakes} warning/><GuideSteps title="FINISH SAFELY" items={[guide.finish]}/></> : null}
                  <Text style={styles.guideSafetyNote}>Use a controlled range that feels stable. Stop if you feel sharp pain, dizziness, or cannot keep the movement controlled.</Text>
                </> : null}
              </ScrollView>
  ...
  return (
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={pullRefresh} colors={[colors.primary]} tintColor={colors.primary} progressBackgroundColor={colors.panel}/>} contentContainerStyle={styles.wrap} keyboardShouldPersistTaps="handled">
        ...
        {muscleFilter === 'All' && !searchMode && !query ? <View style={[styles.muscleGrid,styles.muscleGridFirst]}>{[
...
function DraggableOrderRow({item,index,count,isCurrent,disabled,onMove,onMoveNext,onMoveEnd}:{item:BuilderItem;index:number;count:number;isCurrent:boolean;disabled:boolean;onMove:(from:number,to:number)=>void;onMoveNext:()=>void;onMoveEnd:()=>void}){
  ...
  const responder=React.useMemo(()=>PanResponder.create({
    ...
    onPanResponderRelease:(_,gesture)=>{
      const target=Math.max(0,Math.min(count-1,index+Math.round(gesture.dy/70)));
      Animated.spring(translateY,{toValue:0,useNativeDriver:true}).start(()=>{if(target!==index)onMove(index,target);});
    },
  ...
  const complete=item.done||(item.exercise.metric_type==='strength'&&item.strengthSets.length>0&&item.strengthSets.every((set)=>set.done));
  return <Animated.View style={[styles.reorderRow,isCurrent&&styles.reorderCurrent,{transform:[{translateY}]}]}>
    <View {...responder.panHandlers} style={[styles.dragGrip,disabled&&{opacity:.28}]} accessibilityRole="button" accessibilityLabel={`Drag ${item.exercise.name}`}><Text style={styles.dragGripText}>☷</Text></View>
    <View style={styles.reorderNumber}><Text style={styles.reorderNumberText}>{index+1}</Text></View>
    <View style={{flex:1}}><Text style={styles.reorderName}>{item.exercise.name}</Text><Text style={styles.reorderMeta}>{isCurrent?'Current exercise':complete?'Complete':'Pending'}</Text></View>
    <Pressable disabled={disabled||isCurrent||count<2} onPress={onMoveNext} style={styles.reorderQuick}><Text style={styles.reorderQuickText}>NEXT</Text></Pressable>
    <Pressable disabled={disabled||index===count-1} onPress={onMoveEnd} style={styles.reorderQuick}><Text style={styles.reorderQuickText}>END</Text></Pressable>
  </Animated.View>;
}

function GuideSteps({title,items,warning=false}:{title:string;items:string[];warning?:boolean}){
  const {colors,isDark}=useTheme();const styles=createStyles(colors,isDark);
  return <View style={styles.guideBlock}><Text style={styles.guideSectionTitle}>{title}</Text>{items.map((item,index)=><View key={`${title}-${index}`} style={[styles.guideCueRow,warning&&styles.guideWarningRow]}><Text style={[styles.guideCueNumber,warning&&styles.guideWarningNumber]}>{warning?'!':index+1}</Text><Text style={styles.guideCueText}>{item}</Text></View>)}</View>;
}

type MovementGuide={setup:string[];perform:string[];breathing:string;mistakes:string[];finish:string};
function movementGuide(exercise: LibraryExercise):MovementGuide {
  const name = exercise.name.toLowerCase();
  if (exercise.targetArea === 'Cardio') return {setup:[`Adjust the ${exercise.equipment.toLowerCase()} so the position feels secure and controls are within easy reach.`,'Begin gently for a few minutes before increasing pace or resistance.'],perform:['Keep a tall, relaxed posture and use a smooth rhythm you can repeat.','Change speed, incline, or resistance gradually rather than suddenly.','Record the displayed distance and time when the effort is complete.'],breathing:'Breathe continuously at a steady rhythm; avoid holding your breath during harder efforts.',mistakes:['Starting too fast before the body is ready.','Leaning heavily on handles or letting posture collapse.'],finish:'Reduce the pace gradually, come to a stable stop, and step off only when the equipment is no longer moving.'};
  if (/deadlift|good morning|row/.test(name)) return {setup:['Place the feet securely, keep the load close, and take a balanced grip.','Brace the trunk and set a long, neutral spine before moving the weight.'],perform:['Drive through the floor while the hips and shoulders rise together.','Keep the load close and finish tall without leaning backward.','Return under control, then reset the same starting position.'],breathing:'Take a breath and brace before the effort; breathe out after passing the hardest part, then reset.',mistakes:['Rounding the back or letting the load drift away.','Jerking from the floor or rushing the controlled return.'],finish:'Complete the final repetition with control and place the weight securely before releasing your grip.'};
  if (/squat|lunge|step-up|split squat/.test(name)) return {setup:['Use a stance that keeps the whole foot supported and your balance centred.','Brace gently and point the knees in the same direction as the toes.'],perform:['Bend through the hips and knees while keeping the foot planted.','Move only through a range you can control without losing position.','Push through the supported foot and finish standing stable.'],breathing:'Breathe in and brace before lowering; breathe out as you rise through the hardest part.',mistakes:['Knees collapsing inward or heels lifting from the floor.','Dropping quickly into the bottom or bouncing out of position.'],finish:'Stand balanced, then rack or lower the weight using the same controlled stance.'};
  if (/press|push-up|dip|fly/.test(name)) return {setup:[`Set the ${exercise.equipment.toLowerCase()} securely and place the shoulders in a stable position.`,'Keep the wrists stacked and feet or contact points firmly supported.'],perform:['Lower with control until reaching a comfortable, repeatable range.','Press or bring the arms together without losing the shoulder position.','Finish smoothly without forcing the elbows or shoulders past comfort.'],breathing:'Breathe in during the controlled lowering phase and breathe out during the press or effort.',mistakes:['Flaring or shifting the shoulders to chase extra range.','Bouncing the load or letting the wrists fold backward.'],finish:'Return the load to its supports carefully and confirm it is secure before letting go.'};
  if (/pull-up|pulldown|curl|pull|raise/.test(name)) return {setup:['Choose a stable grip and set the shoulders away from the ears.','Brace the torso so the intended joint can move without body swing.'],perform:['Lead with the elbows or working limb and keep the path controlled.','Pause briefly where the target muscles are working most.','Return slowly until the start position is reached without losing tension.'],breathing:'Breathe out during the pull or lift and breathe in during the controlled return.',mistakes:['Using momentum, leaning back, or shrugging to move more weight.','Dropping the resistance quickly between repetitions.'],finish:'Return the handle or weight gently and release it only after it is supported.'};
  if (/plank|crunch|sit-up|twist|wheel|wiper|core/.test(name) || exercise.targetArea === 'Core') return {setup:['Use a stable floor or bench position and set the trunk before moving.','Place the neck and shoulders comfortably; avoid pulling on the head.'],perform:['Move slowly through the intended range while keeping the trunk controlled.','Pause briefly at the working position without holding tension in the neck.','Return smoothly and reset before the next repetition.'],breathing:'Breathe out during the effort and breathe in as you return; do not hold your breath for repeated reps.',mistakes:['Swinging the limbs or rushing to create momentum.','Continuing after the lower back, neck, or shoulders lose position.'],finish:'Stop while you can still control the position, then lower the body or equipment gently.'};
  return {setup:[`Check that the ${exercise.equipment.toLowerCase()} is stable and adjusted for ${exercise.name}.`,'Match the starting position shown and begin with a resistance you can control.'],perform:['Move through a smooth, comfortable range without rushing.','Keep the working joints aligned and the rest of the body stable.','Pause briefly, then return to the start under control.'],breathing:'Breathe continuously—usually out during the effort and in during the easier return.',mistakes:['Using momentum or changing position to force the repetition.','Choosing resistance that prevents a consistent range and tempo.'],finish:'Complete the final repetition slowly and return all equipment to a secure resting position.'};
}
...
const createStyles = (colors: any, isDark: boolean) => StyleSheet.create({
  ...
  orderHintRow:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginBottom:7},orderHint:{color:colors.muted,fontSize:9},orderButton:{minHeight:34,borderRadius:9,borderWidth:1,borderColor:colors.primary,paddingHorizontal:10,alignItems:'center',justifyContent:'center'},orderButtonText:{color:colors.primary,fontSize:8,fontWeight:'900'},
  ...
  reorderSheet:{height:'82%',backgroundColor:colors.bg,borderTopLeftRadius:24,borderTopRightRadius:24,borderWidth:1,borderColor:colors.border,padding:16},reorderList:{gap:8,paddingVertical:8,paddingBottom:18},reorderRow:{minHeight:64,flexDirection:'row',alignItems:'center',gap:7,borderRadius:13,borderWidth:1,borderColor:colors.border,backgroundColor:colors.panel,paddingHorizontal:8,shadowColor:'#000',shadowOpacity:.08,shadowRadius:5,shadowOffset:{width:0,height:2},elevation:2},reorderCurrent:{borderColor:colors.primary,backgroundColor:colors.primarySoft},dragGrip:{width:34,height:48,alignItems:'center',justifyContent:'center'},dragGripText:{color:colors.muted,fontSize:25,fontWeight:'900'},reorderNumber:{width:25,height:25,borderRadius:13,backgroundColor:colors.panel2,alignItems:'center',justifyContent:'center'},reorderNumberText:{color:colors.primary,fontSize:9,fontWeight:'900'},reorderName:{color:colors.text,fontSize:11,fontWeight:'900'},reorderMeta:{color:colors.muted,fontSize:8,marginTop:3},reorderQuick:{minHeight:37,minWidth:42,borderRadius:8,borderWidth:1,borderColor:colors.border,alignItems:'center',justifyContent:'center',paddingHorizontal:5},reorderQuickText:{color:colors.primary,fontSize:7,fontWeight:'900'},
  ...
  guideBlock:{marginBottom:9},guideSectionTitle: { color: colors.text, fontSize: 13, fontWeight: '900', marginBottom: 8 },
  ...
  guideWarningRow:{backgroundColor:colors.panel2,borderRadius:11,padding:8},guideWarningNumber:{backgroundColor:colors.danger,color:'#FFFFFF'},
