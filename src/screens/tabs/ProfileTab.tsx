...
const ProfileTab = forwardRef<ProfileTabHandle, Props>(function ProfileTab({ profile, onProfileChanged, onOpenCustomization, onOpenSupplements, onOpenSplit, onOpenClubs, onOpenJourney, onOpenSharedGym }, ref) {
  ...
  useEffect(()=>{
    ...
    setFitness(profile.fitness_level??'new'); setActivity(profile.activity_level??'light'); setGender(profile.gender??'prefer_not_to_say'); setGoal(profile.goal??'improve_fitness'); setDays(String(profile.workout_days_target??3));
  ...
  const saveDetails=async()=>{
    ...
    if(!a||a<13||a>100||!w||w<=0||!h||h<=0||!d||d<1||d>7)return Alert.alert('Check your details','Enter valid age, height, weight and training days.');
    ...
    if(error&&/height_unit|weight_unit|column/i.test(error.message)){delete payload.height_unit;delete payload.weight_unit;({error}=await supabase.from('profiles').update(payload).eq('id',profile.id));}
    setBusy(false);
