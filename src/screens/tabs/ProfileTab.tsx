...
import { ClubsSceneIcon, CustomizePhoneSceneIcon, JourneyProgressSceneIcon, ProfileDetailIcon, ProfileDetailIconKind, ProfileIdSceneIcon, ProfileSceneIcon, SharedWorkoutIcon, SupplementTrackerSceneIcon, WeeklySplitSceneIcon, WorkoutBuilderSceneIcon } from '../../components/FitHubTrackerIcons';
...
const ProfileTab = forwardRef<ProfileTabHandle, Props>(function ProfileTab({ profile, onProfileChanged, onOpenCustomization, onOpenSupplements, onOpenSplit, onOpenClubs, onOpenJourney, onOpenSharedGym }, ref) {
  ...
  return <RefreshableScrollView onRefresh={onProfileChanged} contentContainerStyle={styles.wrap} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.accountGrid}>
      <AccountArtCard icon={<CustomizePhoneSceneIcon size={96} color={colors.text} accentColor={colors.primary} surfaceColor={colors.panel}/>} title="Customize FitHub" sub={`Theme, colours and units · ${themeKey.replace(/([A-Z])/g,' $1').trim()}`} onPress={onOpenCustomization}/>
      <AccountArtCard icon={<WeeklySplitSceneIcon size={96} color={colors.text} accentColor={colors.primary} surfaceColor={colors.panel}/>} title="Weekly split" sub="Plan training and recovery days" onPress={onOpenSplit}/>
      {!hiddenFeatures.includes('clubs')?<AccountArtCard wide icon={<ClubsSceneIcon size={104} color={colors.text} accentColor={colors.primary} surfaceColor={colors.panel}/>} title="Clubs" sub="Community milestones, challenges and progress" onPress={onOpenClubs}/>:null}
    </View>
