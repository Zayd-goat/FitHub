...
import { Card, Input, RefreshableScrollView, SectionTitle, useTheme } from '../components/UI';
...
export default function ProgressScreen({ profile, focus = 'overview', onBack }: { profile: Profile; focus?: ProgressFocus; onBack: () => void }) {
  ...
  const currentAge = profileAge(profile) ?? 0;
  ...
  const nutritionDays = useMemo(() => {
    if (currentAge < 18 || !profile.maintenance_calories || !profile.protein_target_g) return { calorie: 0, protein: 0, both: 0 };
  ...
  }, [foods, currentAge, profile.maintenance_calories, profile.protein_target_g]);
  ...
  }, [profile.login_streak, profile.workout_streak, currentAge, sessions.length, foods.length, nutritionDays, allPrCount]);
  ...
  const badgeSection = <>
    ...
    {currentAge >= 18 ? <Card><SectionTitle title="Nutrition achievements" subtitle="Based on your current adult targets." /><Info label="Energy-target days" value={`${nutritionDays.calorie}`} /><Info label="Protein-target days" value={`${nutritionDays.protein}`} /><Info label="Both targets in one day" value={`${nutritionDays.both}`} /></Card> : null}
  </>;
