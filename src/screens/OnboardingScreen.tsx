...
export default function OnboardingScreen({ profile, onComplete }: { profile: Profile; onComplete: () => void }) {
  ...
  const valid = useMemo(() => {
    ...
    if (step === 2) { const n = Number(age); return Number.isFinite(n) && n >= 13 && n <= 100; }
  ...
  }, [step, fitness, activity, age, gender, height, weight, goal, days]);
  ...
  const finish = async () => {
    ...
    if (error && /height_unit|weight_unit|column/i.test(error.message)) {
      ...
      <KeyboardAvoidingView style={styles.safe} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={styles.wrap}>
          ...
          <View style={styles.questionArea}>
            ...
            {step === 2 && <Question title="How old are you?" subtitle="Age helps FitHub apply the right safety settings."><Input value={age} onChangeText={setAge} keyboardType="number-pad" placeholder="Age" /></Question>}
            ...
            {step === 6 && <Question title="What is your main training goal?" subtitle="You can change this later whenever your goals change."><ChoiceRow values={[['gain_muscle','Gain muscle'],['fat_loss','Lose fat'],['maintain','Maintain'],['improve_fitness','Improve fitness']]} value={goal} onChange={setGoal} /></Question>}
