...
export default function AuthScreen() {
  ...
  const requestPasswordReset = async () => {
    if (!cleanEmail || !cleanEmail.includes('@')) return Alert.alert('Enter your email', 'Enter the email address connected to your FitHub account.');
    setBusy(true);
    try {
  ...
  const resendConfirmation = async () => {
    if (!cleanEmail || !cleanEmail.includes('@')) return Alert.alert('Enter your email', 'Enter the email address used to create your FitHub account.');
    setBusy(true);
    try {
  ...
  const submit = async () => {
    if (!cleanEmail || !password || (mode === 'signup' && !username.trim())) {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.wrap} keyboardShouldPersistTaps="handled">
          ...
          <Card style={{ width: '100%' }}>
            ...
            <Text style={styles.muted}>{mode === 'signin' ? 'Sign in to continue your training.' : mode === 'signup' ? 'Your profile will sync securely across devices.' : mode === 'forgot' ? 'We will email you a secure link to choose a new password.' : `We sent a confirmation link to ${cleanEmail || 'your email address'}.`}</Text>

            {mode === 'confirmation' ? <>
              <Text style={styles.confirmationCopy}>Open the newest FitHub email on this phone and tap “Confirm my FitHub account.” You can then return to the app.</Text>
