...
const clubThresholds: Array<{ aliases: string[]; canonical: string; thresholds: number[] }> = [
  { aliases: ['bench press','benchpress','flat bench press','barbell bench press','barbell benchpress','barbell flat bench press'], canonical: 'Barbell Bench Press', thresholds: [40,60,80,100,120] },
  { aliases: ['back squat','barbell squat','barbell back squat','high bar squat','high bar back squat','low bar squat','low bar back squat'], canonical: 'Barbell Back Squat', thresholds: [60,100,140,180] },
  { aliases: ['deadlift','conventional deadlift','barbell deadlift','barbell conventional deadlift'], canonical: 'Conventional Deadlift', thresholds: [80,120,160,200] },
  { aliases: ['overhead press','barbell overhead press','standing overhead press','military press','barbell military press'], canonical: 'Overhead Press', thresholds: [30,50,70,90] },
];
const slug=(value:string)=>value.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');
const normalizeLift=(value:string)=>value.toLowerCase().replace(/[^a-z0-9]+/g,' ').trim().replace(/\s+/g,' ');
...
async function unlockClubs(userId:string, events:any[]) {
  ...
  for (const event of events) {
    ...
    const normalized=normalizeLift(event.exercise_name);
    const definition=clubThresholds.find((x)=>x.aliases.includes(normalized));
