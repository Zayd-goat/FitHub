...
const GYM_INVITE_CATEGORY = 'GYMINVITE';
...
export const GYM_INVITE_ACCEPT_ACTION = 'GYM_INVITE_ACCEPT';
export const GYM_INVITE_DECLINE_ACTION = 'GYM_INVITE_DECLINE';
...
async function finalizeStoredWorkout(userId: string) {
  ...
  try {
    ...
    if (insertedRows.length) {
      const { data: profileRow } = await supabase.from('profiles').select('age,date_of_birth').eq('id', userId).maybeSingle();
      newPrs = await detectAndSavePrEvents({ userId, sessionId: session.id, rows: insertedRows, age: profileAge(profileRow) });
      if (newPrs.length) {
...
function responseNotificationId(data: any) {
  return String(data?.notification?.request?.identifier ?? data?.notification?.identifier ?? '');
}
...
if (!TaskManager.isTaskDefined(WORKOUT_NOTIFICATION_TASK)) {
  TaskManager.defineTask<Notifications.NotificationTaskPayload>(WORKOUT_NOTIFICATION_TASK, async ({ data, error }) => {
    ...
    const notificationId = responseNotificationId(payload);
    if (!userId) {
    ...
    if (action === SUPPLEMENT_TAKEN_ACTION && content?.reminderId) {
      ...
      try { await supabase.from('supplement_checkins').upsert({user_id:userId,reminder_id:String(content.reminderId),local_date:localDate,taken_at:now.toISOString(),status:'taken',recorded_time:now.toTimeString().slice(0,8),source:'notification',updated_at:now.toISOString()},{onConflict:'user_id,reminder_id,local_date'}); } catch {}
      await cancelSameDaySupplementReschedule(String(content.reminderId),now);
    }
    if ((action === GYM_INVITE_ACCEPT_ACTION || action === GYM_INVITE_DECLINE_ACTION) && content?.inviteId) {
      const status = action === GYM_INVITE_ACCEPT_ACTION ? 'accepted' : 'declined';
      try {
        await supabase.from('gym_invites').update({ status, updated_at: new Date().toISOString() }).eq('id', String(content.inviteId)).eq('recipient_id', userId).eq('status', 'pending');
        if (status === 'accepted') await scheduleAcceptedGymReminderForInvite(String(content.inviteId), userId);
        await supabase.functions.invoke('friend-notifications', { body: { invite_id: String(content.inviteId), notification_kind: 'response' } }).catch(() => null);
      } catch {}
    }
    if (notificationId) await Notifications.dismissNotificationAsync(notificationId).catch(() => {});
  });
...
export async function ensureNotificationSetup(requestPermission = true) {
  if (Platform.OS === 'android') {
    ...
    await Notifications.setNotificationChannelAsync(GYM_CHANNEL, {
      name: 'Gym invites & session reminders',
  ...
  await Notifications.setNotificationCategoryAsync(GYM_INVITE_CATEGORY, [
    { identifier: GYM_INVITE_ACCEPT_ACTION, buttonTitle: 'ACCEPT', options: { opensAppToForeground: false, isAuthenticationRequired: false, isDestructive: false } },
    { identifier: GYM_INVITE_DECLINE_ACTION, buttonTitle: 'DECLINE', options: { opensAppToForeground: false, isAuthenticationRequired: false, isDestructive: true } },
  ]).catch(() => null);
...
export async function registerFriendPushToken(userId:string){
  ...
  if(!allowed)return {ok:false,reason:'Notification permission was not granted.'};
  try{
    const projectId=process.env.EXPO_PUBLIC_EXPO_PROJECT_ID ?? Constants.expoConfig?.extra?.eas?.projectId ?? Constants.easConfig?.projectId;
    if(!projectId)return {ok:false,reason:'EXPO_PUBLIC_EXPO_PROJECT_ID is missing.'};
    const token=(await Notifications.getExpoPushTokenAsync({projectId})).data;
    if(!token)return {ok:false,reason:'Expo did not return a push token.'};
    const {error}=await supabase.from('push_tokens').upsert({user_id:userId,token,platform:Platform.OS,enabled:true,updated_at:new Date().toISOString()},{onConflict:'user_id,token'});
    if(error)throw error;
    return {ok:true,reason:null};
  }catch(error:any){
    return {ok:false,reason:String(error?.message??'Push registration failed.')};
  }
}
...
export async function scheduleAcceptedGymReminderForInvite(inviteId: string, userId: string) {
  const { data: invite, error } = await supabase
    .from('gym_invites')
    .select('id,sender_id,recipient_id,session_at,gym_name,workout_name,status')
    .eq('id', inviteId)
    .maybeSingle();
  if (error || !invite || invite.status !== 'accepted') return null;
  if (invite.sender_id !== userId && invite.recipient_id !== userId) return null;

  const otherUserId = invite.sender_id === userId ? invite.recipient_id : invite.sender_id;
  const { data: other } = await supabase.from('public_profiles').select('username').eq('user_id', otherUserId).maybeSingle();
  return scheduleGymReminder({
    inviteId,
