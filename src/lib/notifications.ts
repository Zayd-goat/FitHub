...
export async function registerFriendPushToken(userId:string){
  ...
  try{
    ...
    if(token)await supabase.from('push_tokens').upsert({user_id:userId,token,platform:Platform.OS,enabled:true,updated_at:new Date().toISOString()},{onConflict:'user_id,token'});
  }catch{}
}
