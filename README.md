...
FitHub is an Expo/React Native fitness app using the supplied FitHub artwork as the Android/iOS app icon. The project includes:
...
Install Node.js 24, then from the project folder run:
...
The GitHub workflow deliberately uses **Node 24** and `actions/upload-artifact@v4`, and it explicitly copies the generated APK to `FitHub.apk`, so an **Artifacts** section is created when the build succeeds.
