...
function normalized(food:any){const ss=array(food.servings?.serving),s=ss.find((x:any)=>x.is_default==='1')??ss[0]??{};return{provider_food_id:String(food.food_id),name:food.food_name,brand:food.brand_name??null,source:'FatSecret verified database',serving_id:s.serving_id?String(s.serving_id):null,serving:s.serving_description??'1 serving',calories:Number(s.calories??0),protein_g:Number(s.protein??0),carbs_g:Number(s.carbohydrate??0),fat_g:Number(s.fat??0),fibre_g:Number(s.fiber??0),available_servings:ss.map((x:any)=>({serving_id:String(x.serving_id),label:x.serving_description,calories:Number(x.calories??0),protein_g:Number(x.protein??0),carbs_g:Number(x.carbohydrate??0),fat_g:Number(x.fat??0),fibre_g:Number(x.fiber??0)}))};}
serve(async req=>{
