...
Deno.serve(async (req) => {
  ...
  if (cronRequest) {
    ...
    const { data: invite, error: inviteError } = await db.from('gym_invites').select('id,sender_id,status').eq('id', inviteId).maybeSingle();
    if (inviteError) return Response.json({ error: inviteError.message }, { status: 500 });
    if (!invite || invite.sender_id !== authData.user.id) return Response.json({ error: 'Invite not found' }, { status: 404 });
    const { data: queued, error: queueError } = await db.from('gym_invite_notification_outbox').select('id,invite_id,recipient_id,actor_id,retry_count').eq('invite_id', inviteId).is('processed_at', null).maybeSingle();
  ...
  return Response.json({ processed, friendQueued: friendQueue.length, inviteQueued: inviteQueue.length, mode: cronRequest ? 'scheduled' : 'immediate-invite' });
});
